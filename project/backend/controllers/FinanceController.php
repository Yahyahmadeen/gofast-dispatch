...
class FinanceController
{
    ...
    public function initializePayment():void{
        ...
        $amount=(float)$o['delivery_fee'];if($amount<=0){$this->respond(false,'This order has no online payment amount',400);return;}
        $reference='GF-PAY-'.date('YmdHis').'-'.strtoupper(bin2hex(random_bytes(3)));
        try{
            $r=paystackRequest('POST','/transaction/initialize',['email'=>$u['email'],'amount'=>(int)round($amount*100),'currency'=>'NGN','reference'=>$reference,'callback_url'=>rtrim(gofastEnv('GOFAST_APP_URL','http://localhost:5174'),'/').'/customer/orders','metadata'=>['order_id'=>$orderId,'customer_user_id'=>(int)$u['id']]]);
    ...
    public function riderWallet():void{
        ...
        $q=$this->db->prepare("SELECT COALESCE((SELECT SUM(CASE WHEN type IN ('earning','adjustment') THEN amount ELSE 0 END) FROM rider_wallet_transactions WHERE rider_user_id=:user1),0)-COALESCE((SELECT SUM(CASE WHEN type='payout' THEN amount ELSE 0 END) FROM rider_wallet_transactions WHERE rider_user_id=:user2),0)-COALESCE((SELECT SUM(amount) FROM payout_requests WHERE rider_user_id=:user3 AND status IN ('pending','approved')),0) balance,COALESCE((SELECT SUM(CASE WHEN type IN ('earning','adjustment') THEN amount ELSE 0 END) FROM rider_wallet_transactions WHERE rider_user_id=:user4),0) earned,COALESCE((SELECT SUM(CASE WHEN type='payout' THEN amount ELSE 0 END) FROM rider_wallet_transactions WHERE rider_user_id=:user5),0) paid");$q->execute(['user1'=>$u['id'],'user2'=>$u['id'],'user3'=>$u['id'],'user4'=>$u['id'],'user5'=>$u['id']]);$summary=$q->fetch()?:[];
    ...
    public function requestPayout():void{
        ...
        $b=$this->db->prepare("SELECT COALESCE((SELECT SUM(CASE WHEN type IN ('earning','adjustment') THEN amount ELSE 0 END) FROM rider_wallet_transactions WHERE rider_user_id=:user1),0)-COALESCE((SELECT SUM(CASE WHEN type='payout' THEN amount ELSE 0 END) FROM rider_wallet_transactions WHERE rider_user_id=:user2),0)-COALESCE((SELECT SUM(amount) FROM payout_requests WHERE rider_user_id=:user3 AND status IN ('pending','approved')),0)");$b->execute(['user1'=>$u['id'],'user2'=>$u['id'],'user3'=>$u['id']]);$balance=(float)$b->fetchColumn();
        if($amount>$balance){$this->respond(false,'Requested amount exceeds your available balance',400);return;}
        $a=$this->db->prepare("SELECT status FROM rider_payout_accounts WHERE rider_user_id=:user LIMIT 1");$a->execute(['user'=>$u['id']]);$account=$a->fetch();if(!$account){$this->respond(false,'Add a payout bank account first',400);return;}if(($account['status']??'pending')!=='verified'){$this->respond(false,'Your payout bank account must be verified before requesting a payout',403);return;}
    ...
    public function adminFinanceSummary():void{
        $u=$this->requireRole(['admin']);if(!$u)return;
        $q=$this->db->query("SELECT COUNT(*) total_requests, COALESCE(SUM(CASE WHEN status='pending' THEN amount ELSE 0 END),0) pending_amount, COALESCE(SUM(CASE WHEN status='paid' THEN amount ELSE 0 END),0) paid_amount, COALESCE(SUM(CASE WHEN status='rejected' THEN amount ELSE 0 END),0) rejected_amount FROM payout_requests");
        $summary=$q->fetch()?:[];
        $p=$this->db->query("SELECT p.id,p.amount,p.status,p.requested_at,p.processed_at,u.full_name rider_name FROM payout_requests p JOIN users u ON u.id=p.rider_user_id ORDER BY p.requested_at DESC LIMIT 50");
        $this->respond(true,'Finance summary retrieved',200,['summary'=>$summary,'payouts'=>$p->fetchAll()]);
    }
    ...
    public function processPayout():void{
        ...
        if($action==='reject' && !in_array($p['status'],['pending','approved'],true)){$this->respond(false,'This payout can no longer be rejected',409);return;}
        if($action==='approve' && $p['status']!=='pending'){$this->respond(false,'Only pending payouts can be approved',409);return;}
        if($action==='paid' && !in_array($p['status'],['pending','approved'],true)){$this->respond(false,'Only pending or approved payouts can be marked paid',409);return;}
